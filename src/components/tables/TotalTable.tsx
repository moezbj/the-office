...
export default function TotalTable({ data, user, fees }: TableProps) {
    ...
    const resFees =
        fees.length > 0
            ? fees.map((e) => e.amount).reduce((acc, item) => item + acc)
            : 0;
    ...
    const invoiceTaxes = invoiceSubtotal - resFees;
    const invoiceTotal = invoiceTaxes;
        ...
        <div className="flex flex-col w-full" dir={document.documentElement.dir}>
            ...
            >
                ...
                <div className="mb-8 space-y-1 text-gray-900">
                    ...
                    <p className="text-base">
                        <span className="font-semibold">taxe:</span> {user.taxRegistration}
                    </p>
                </div>
                    ...
                    <table className="w-full text-sm text-left text-gray-900">
                        <thead className="bg-gray-50 border-b border-gray-200 text-xs uppercase font-semibold text-gray-700">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-right">somme</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            ...
                            <tr className="bg-gray-50 font-semibold">
                                ...
                                <td className="px-4 py-3 text-right" colSpan={2}>sous total</td>
                                <td className="px-4 py-3 text-right font-medium">{ccyFormat(invoiceSubtotal)}</td>
                            </tr>

                            {/* Fee Row */}
                            <tr className="bg-gray-50 font-semibold">
                                <td className="px-4 py-3">Dépence</td>
                                <td className="px-4 py-3 text-right">{`${resFees}dt`}</td>
                                <td className="px-4 py-3 text-right font-medium">{ccyFormat(invoiceTaxes)}</td>
                            </tr>

                            {/* Total Row */}
                            <tr className="bg-gray-100 font-bold text-base border-t-2 border-gray-300">
                                <td className="px-4 py-4 text-right" colSpan={2}>Total</td>
